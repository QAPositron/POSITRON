                ...
                <div class="row">
                    ...
                    <div class="col">
                        <div class="d-grid gap-2 col-6 mx-auto">
                            @php
                                $totalAreas = count($dosiareamesact);
                                $totalAreasT = 0;
                                $totalTorax = count($dositoraxmesact);
                                $totalToraxT = 0;
                                $totalCaso = count($dosicasomesact);
                                $totalCasoT = 0;
                                $totalCristalino = count($dosicristalinomesact);
                                $totalCristalinoT = 0;
                                $totalDedo = count($dosidedomesact);
                                $totalDedoT = 0;
                            @endphp
                            @foreach($dosiareamesact as $areas)
                                @if($areas->dosimetro_uso == 'FALSE')
                                    @php $totalAreasT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dositoraxmesact as $torax)
                                @if($torax->dosimetro_uso == 'FALSE')
                                    @php $totalToraxT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosicasomesact as $caso)
                                @if($caso->dosimetro_uso == 'FALSE')
                                    @php $totalCasoT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosicristalinomesact as $cristalino)
                                @if($cristalino->dosimetro_uso == 'FALSE')
                                    @php $totalCristalinoT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosidedomesact as $dedo)
