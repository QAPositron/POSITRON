                ...
                <div class="row align-items-center ">
                    ...
                    <div class="col-md-2">
                        <div class="d-grip gap-2 text-center">
                            @php
                                $totalAreas = count($dosiareamesant);
                                $totalAreasT = 0;
                                $totalTorax = count($dositoraxmesant);
                                $totalToraxT = 0;
                                $totalCaso = count($dosicasomesant);
                                $totalCasoT = 0;
                                $totalCristalino = count($dosicristalinomesant);
                                $totalCristalinoT = 0;
                                $totalDedo = count($dosidedomesant);
                                $totalDedoT = 0;
                            @endphp
                            @foreach($dosiareamesant as $areas)
                                @if($areas->dosimetro_uso == 'FALSE')
                                    @php $totalAreasT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dositoraxmesant as $torax)
                                @if($torax->dosimetro_uso == 'FALSE')
                                    @php $totalToraxT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosicasomesant as $caso)
                                @if($caso->dosimetro_uso == 'FALSE')
                                    @php $totalCasoT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosicristalinomesant as $cristalino)
                                @if($cristalino->dosimetro_uso == 'FALSE')
                                    @php $totalCristalinoT += 1;  @endphp
                                @endif
                            @endforeach
                            @foreach($dosidedomesant as $dedo)
                                @if($dedo->dosimetro_uso == 'FALSE')
                                    @php $totalDedoT += 1;  @endphp
                                @endif
                            @endforeach
                            @if($totalAreas == $totalAreasT && $totalTorax == $totalToraxT && $totalCaso == $totalCasoT && $totalCristalino == $totalCristalinoT && $totalDedo == $totalDedoT)
